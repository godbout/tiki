...
*/
				var vectorLayerList = [container.vectors];
				...
				if ($(container).data("tooltips")) {
					...
					var $mapTooltip = $("<div>").attr("id", "map-tooltip").appendTo($(".ol-viewport", container));
...
/*		work in progress
				...
				select.on('select', function (e) {
					...
					$(container).setMapPopup(features);
				});
	...
	$.fn.loadInfoboxPopup = function (options) {
		if (options.type && options.object && $.inArray(options.type,  jqueryTiki.infoboxTypes) !== -1) {
			...
			this.each(function () {
				...
				popup = new ol.Popup('marker',  ol.proj.fromLonLat(options.lonlat), null, options.content + '<img src="img/spinner.gif"/>');
				popup.autoSize = true;
				$(container).setMapPopup(popup);
				...
				$.get($.service('object', 'infobox', {
					...
					newPopup = handler.create(ol.proj.fromLonLat(options.lonlat), content, options.hook, close, options.feature);
