					...
					_createMarker: function (name, lonlat, callback) {
						if (lonlat) {
							if (typeof lonlat === "array") {
								lonlat = new ol.geom.Point(ol.proj.fromLonLat([lonlat.lon, lonlat.lat]));
							}
...
/*							container.map.setLayerZIndex(vectors, vectorLayerList.length * 1000);
				...
				if (central) {
					container.resetPosition = function () {
						map.setView(new ol.View({
							center: ol.proj.fromLonLat([central.lon, central.lat]),
							zoom: central.zoom
						}));
					};
	...
	$.fn.addMapMarker = function (options) {
		this.each(function () {
			...
			if (options.lat && options.lon) {
				lonlat = new ol.geom.Point( ol.proj.fromLonLat([options.lon, options.lat]));
			}
