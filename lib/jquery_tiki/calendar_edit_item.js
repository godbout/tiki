...
$(document).ready(function () {
	...
	$('select[name="save[participants][]"]').change(function() {
		...
		for (var i = 0, l = users.length; i < l; i++) {
			if ($('#participant_roles tr[data-user="'+users[i]+'"]').length == 0) {
				sel = $('<select/>').attr('name', 'save[participant_roles]['+users[i]+']')
				...
				$('#participant_roles').append('<tr data-user="'+users[i]+'"><td>'+users[i]+'</td><td></td><td>'+sel[0].outerHTML+'</td></tr>');
			}
		}
	});
