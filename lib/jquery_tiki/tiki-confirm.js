...
/**
 *	See checkTimeout() documentation below. check-timeout class can used for a form so that the user is warned that
 *  the ticket is timed out before entering data into the form.	This may not work for forms in popups (e.g., tooltips)
 *  but the checkTimeout() onclick method below will
 */
$('form.check-timeout input, form.check-timeout select').on('mousedown keydown', function() {
	if (checkTimeout()) {
		return true;
	}
});
