...
/**
 * Utility that checks whether the security ticket has timed out used in function below
 *
 * @returns {boolean}
 */
...
$.fn.applyTicketTimeout = function () {
	...
	$('form').has('input[name=ticket]')
		.on('mousedown keydown', 'select, input, textarea, button[type=submit]', $.fn.ticketTimeout);
}
 ...
 * applyTicketTimeout doesn't work. Modals are not handled
 */
$("[data-toggle='popover']").on('shown.bs.popover', function() {
	// set what happens when user clicks on the button
	$('form').has('input[name=ticket]').on('click', '[type=submit]', function() {
		// adding a class like
		if (! checkTimeout()) {
			event.preventDefault();
		}
