...
function confirmAction(event) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (dataurl) {
				$.closeModal();
				//compare dataurl input file name to location filename
				var $hreffile = window.location.pathname.substring(window.location.pathname.lastIndexOf('/')),
					$dataurlend = null;
					if (dataurl.lastIndexOf('?') > -1) {
						$dataurlend = dataurl.lastIndexOf('?');
					} else if (dataurl.lastIndexOf('#') > -1) {
						$dataurlend = dataurl.lastIndexOf('#');
					}
					var $dataurlfile = dataurl.substring(dataurl.lastIndexOf('/'), $dataurlend);
				//reload if the same filename, otherwise page won't refresh if only adding a hash anchor to the same page
				if ($hreffile.indexOf($dataurlfile) > -1) {
