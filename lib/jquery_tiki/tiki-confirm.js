 ...
 * is used
 *
 * - for non-ajax forms, typically used for state-changing actions that cannot easily be
 * undone, where a confirmation is advisable.
 * - for ajax forms, captures all form inputs when triggering ajax services modal
 * with a form submission so that the inputs don't need to be added to the
 * bootstrap_modal smarty function as parameters. In this case, it is not being used
 * to produce a confirmation popup since the ajax function should do that
 ...
 * and the confirm test added to a dtat attribute for that option element called
 * data-confirm-text
 *
 ...
 * before entering data into the form. listens for any form and then checks
 * whether the form has the ticket input before performing the check. Doesn't work
 * whe the form is in a popover, hence the next function
 */

$('form').has('input[name=ticket]').on('mousedown keydown', 'select, input, textarea, button[type=submit]', function() {
...
/**
 * Same as above function but altered to work when the form being clicked is on a popover
 */
$("[data-toggle='popover']").on('shown.bs.popover', function() {
	// set what happens when user clicks on the button
	$('form').has('input[name=ticket]').on('click', '[type=submit]', function(e) {
		// don't check timeout again if already check and expired so that the warning only comes up the first time
		// the input element is clicked
		if ($(this).hasClass('already-warned')) {
			return  true;
	...
	// clear listeners
	$('form').has('input[name=ticket]').off('click', '[type=submit]');
});


/**
 * Utility method used by the confirmPopup() method and the timeout warning listeners
 * just above that generates a popup warning and stops the click event if
