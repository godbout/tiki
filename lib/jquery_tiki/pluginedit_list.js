	...
	jqueryTiki.plugins.list = {
		...
		addPlugin: function (plugin, parentPath) {
			...
			if (pluginName === "wiki text") {
				$li.append(
					$("<div>")
						.addClass("params card-body d-flex flex-wrap")
						.append(
							$("<textarea>")
			...
			//$li.find(".card-header > div:first").append($paramsDivs.find(".btn-group").remove().dropdown());
		...
		buildParam: function (pluginName, paramName, value, parentPath, otherParams) {
			...
			return $("<div>")
				...
				.append($("<div>")
					...
					.append(
						$("<div>")
							.addClass("param-name input-group-prepend pr-2")
							.append(
								$moreParamsDropDown,
								$("<span>").text(paramName)
							),
						...
						$("<div>")
							...
							.append(
								$("<a>")
									.addClass("input-group-text text-danger")
									.html("&times;")
									.click(function () {
