...
$.fn.tiki_popover = function () {
	...
	list = $container.find('.tips[title!=""], .tikihelp[title!=""]')
		.each(function () {
			...
			// default Tiki delay
			$element.data('delay', { "show": 0, "hide": 3000 });
		});
	...
	list.filter('.slow').data('delay', { "show": 500, "hide": 3000 });
...
$(document).on('tiki.modal.redraw', '.modal.fade', function () {
	...
	$('.modal-backdrop', modal).slice(1).remove(); // Bootstrap keeps adding more of these
