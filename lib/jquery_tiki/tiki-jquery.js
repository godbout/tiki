...
(function($) {
	...
	$.fn._object_selector_update_results = function (type, result, initial) {
		...
		$.each(result, function (key, value) {
			...
			if (-1 === selection.indexOf(current)) {
				if (initial) {
					$._object_selector_add_item(type, $select, $([]), current, value.title, value.status_icon, false);
				} else {
					$._object_selector_add_item(type, $select, $results, current, value.title, value.status_icon, false);
				}
			} else {
	...
	$.fn.object_selector = function (action, value, title) {
		...
		this.each(function () {
			var input = this
				...
				, searchField = $(input).data('searchfield') || 'content'
			;
	...
	$.fn.object_selector_multi = function (action) {
		...
		this.each(function () {
			var $textarea = $(this).hide().addClass('primary')
				...
				, searchField = $textarea.data('searchfield') || 'content'
				;
	...
	$('.table-responsive').each(function () {
		if ($(this).find('table').width() - 5 > $(this).width()) {
			if ($(this).find('table td.action').length) {
				$(this).find('table td.action').each(function () {
					$(this).parent().prepend($(this).clone());
				});
				$(this).find('table tr').eq(0).prepend('<th style="width:20px;"></th>');
			}
		}
	});
