...
(function($) {
	...
	$('.table-responsive:not(.article-types)').each(function () {
		...
		if (table.find('table:not(.tablesorter) tbody tr').width() - 5 > table.width()) {
			...
			if (screen.width <= 767) {
				$('<div class="toggle-table d-md-none"><button type="button" class="btn btn-link fas fa-toggle-on"></button></div>').insertBefore(table);
				...
				table.find('table th').each(function (e) {
					...
					table.find('table tbody tr').each(function () {
						...
						if (e == 0) {
							if (cell.hasClass('checkbox-cell') && cell.find('.header').html() == '') {
