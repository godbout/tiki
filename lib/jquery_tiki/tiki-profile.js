...
(function ($) {
	...
	$ = $.extend($, {
		...
		previewProfile: function (baseURI, domain, profile, ticket) {
		...
		profilesShowDetails: function ( baseURI, id, domain, profile, ticketValue, ticketValuePreview ) {
			var nid = id + "-sub";
			var infoId = id + "-info";
			var prev = $('#' + id);
			var obj = $('#' + infoId);
			...
			$.post(baseURI + '&getinfo&pd=' + escape(domain) + '&pp=' + escape(profile), function (result) {
				...
				if ( data.installable || data.already ) {
					...
					var ticket = $('<input type="hidden" class="ticket" name="ticket" value="' + ticketValue + '" />');
					form.append(ticket);
					...
					var submitPreview = $('<input type="button" name="profile_preview" value="' + tr('Preview Changes') + '" '
						+ 'onclick="' + "$.previewProfile('" + baseURI + "', '" + domain + "', '" + profile + "', '" + ticketValuePreview + "');" + '"'
