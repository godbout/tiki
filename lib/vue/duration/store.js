var DurationPickerStore = function() {
	return {
		...
		setInitialDuration(duration) {
			...
			if (duration.units.includes("hours") ||
				duration.units.includes("minutes") ||
				duration.units.includes("seconds") ||
				duration.units.includes("milliseconds"))
			{
		...
		setDurationValue(value, unit) {
			this.state.duration.value = moment.duration(this.state.duration.value).add(value, unit);
		},
		setTimestamp(action, timestamp) {
			if (action === 'add') {
				...
				this.state.totalTime = this.state.totalTime.clone().subtract(moment.duration(spentTime));
		...
		updateTimestamp(timestamp) {
			...
			Object.keys(timestamp).forEach(function(key) {
				if (key === 'id') return;
				this.state.timestamps[index][key] = timestamp[key];
			});
		...
		__calcDuration(duration) {
			...
			// Cache formated
			if (!isNegative) this.state.lastPositiveFormated = formated;
			// Replace negative format with last known positive
			if (isNegative) newAmounts = JSON.parse(this.state.lastPositiveFormated);
