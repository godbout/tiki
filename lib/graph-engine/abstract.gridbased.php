...
class GridBasedGraphic extends Graphic // {{{1
{
    ...
    public function _drawContent(&$renderer) // {{{2
    {
        $top = 0;
        $left = 0;
        $bottom = 1;
    ...
    public function _initScales(&$renderer, $layout, $type) // {{{2
    {
    ...
    public function _drawScales(&$renderer, $layout, &$left, &$top, &$right, &$bottom) // {{{2
    {
    ...
    public function _drawGridArea(&$renderer, $layout) // {{{2
    {
...
class GridScale // {{{1
{
    ...
    public function __construct($type, $layout) // {{{2
    {
        $this->type = $type;
        $this->layout = $layout;
    ...
    public function drawScale(&$renderer) // {{{2
    {
        die("Abstract Function Call");
    }

    public function drawGrid(&$renderer) // {{{2
    {
        die("Abstract Function Call");
    }

    public function getLocation($value) // {{{2
    {
        die("Abstract Function Call");
    }

    public function getRange($value) // {{{2
    {
        die("Abstract Function Call");
    }

    public function getSize(&$renderer, $available) // {{{2
    {
        die("Abstract Function Call");
    }
} // }}}1
...
class LinearGridScale extends GridScale // {{{1
{
    ...
    public function __construct($type, $layout, $min, $max) // {{{2
    {
    ...
    public function _adjustScale() // {{{2
    {
        $max = $this->max;
        $min = $this->min;
    ...
    public function _drawGridLine(&$renderer, $pos, $style) // {{{2
    {
    ...
    public function _drawGridTick(&$renderer, $pos, $style, $font, $size, $round = false) // {{{2
    {
        if ($this->orientation == 'vertical') {
    ...
    public function _calculateSkip($size, $space) // {{{2
    {
        $skip = 0;
    ...
    public function _getLargest(&$renderer, $font) // {{{2
    {
        return  max(
    ...
    public function _getMajorInterval() // {{{2
    {
        return 1 / $this->majorScaleCount;
    }

    public function _getMinorInterval() // {{{2
    {
        return 1 / $this->majorScaleCount / $this->minorScaleCount;
    }

    public function _getZeroLocation() // {{{2
    {
    ...
    public function _getValue($pos, $round = false) // {{{2
    {
        $zpos = $this->zero;
...
class StaticGridScale extends GridScale // {{{1
{
    ...
    public function __construct($type, $layout, $labels) // {{{2
    {
        parent::__construct($type, $layout);
        $this->labels = $labels;
    ...
    public function _drawGridTick(&$renderer, $pos, $style, $font, $size) // {{{2
    {
        if ($this->orientation == 'vertical') {
            $size = $size / $renderer->width;
    ...
    public function _getValue($pos) // {{{2
    {
