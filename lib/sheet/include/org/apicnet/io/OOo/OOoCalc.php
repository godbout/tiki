...
class OOoCalc extends absOOo
{
        ...
        if ($tableNode->hasChildNodes()) {
            ...
            if (!$addLign) {
                while ($currentNode != null) {
                    if ($searchRow == $lign) {
                        if ($addCell) {
                            for ($i = $nbCell; $i < $Col - 1; $i++) {
                                $currentNode->appendChild($this->ChildText("table:table-cell", ""));
                            }
    ...
    public function maxChild(&$node)
    {
        ...
        while ($currentNode != null) {
            if ($max < $currentNode->childCount) {
                $max = $currentNode->childCount;
            }
            $currentNode = & $currentNode->nextSibling;
        }
