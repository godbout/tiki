	...
	function editInterfaces()
	{
		...
		if ($trackerId < 1 && $perms->admin == 'y') {
			TikiLib::lib('header')->add_jq_onready(
				<<<JQ
				var addQuestionsButton = $('<span class="button btn btn-primary"><a href="tiki-admin.php?profile=Simple+Wiki+Attributes&repository=&page=profiles&list=List">' + tr('Apply Profile "Simple Wiki Attributes" To Add FutureLink Questions') + '</a></span>')
					.appendTo('#page-bar');
JQ
			);
		} else {
				...
				TikiLib::lib('header')
					...
					->add_jq_onready(
						...
						function genericSingleTrackerItemInterface(type, item) {
							var addButton = $('<span class="button btn btn-primary"><a href="tiki-view_tracker.php?trackerId=' + $trackerId + '">' + tr("Edit FutureLink " + type) + '</a></span>')
								.click(function() {
...
JQ
	...
	function editQuestionsInterface($questions, $trackerId)
	{
		...
		TikiLib::lib('header')
			->add_jq_onready(
				<<<JQ
				var addQuestionsButton = $('<span class="button btn btn-primary"><a href="tiki-view_tracker.php?trackerId=' + $trackerId + '">' + tr("Edit FutureLink Questions") + '</a></span>')
					.click(function() {
