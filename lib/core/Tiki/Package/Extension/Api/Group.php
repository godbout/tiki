...
class Group extends  Api
{
	...
	public static function setTracker($folder, $ref)
	{
		if (strpos($folder, '/') !== false && strpos($folder, '_') === false) {
			$folder = str_replace('/', '_', $folder);
		}
	...
	public static function setPublicCatroot($folder, $ref)
	{
		if (strpos($folder, '/') !== false && strpos($folder, '_') === false) {
			$folder = str_replace('/', '_', $folder);
		}
	...
	public static function setPrivateCatroot($folder, $ref)
	{
		if (strpos($folder, '/') !== false && strpos($folder, '_') === false) {
			$folder = str_replace('/', '_', $folder);
		}
	...
	public static function setManagementPage($folder, $ref)
	{
		if (strpos($folder, '/') !== false && strpos($folder, '_') === false) {
			$folder = str_replace('/', '_', $folder);
		}
	...
	public static function setHomePage($folder, $ref)
	{
		if (strpos($folder, '/') !== false && strpos($folder, '_') === false) {
			$folder = str_replace('/', '_', $folder);
		}
	...
	public function isOrganicGroup($token)
	{
	...
	public function getOrganicGroupBaseName($token)
	{
		$folder = $this->getFolderFromToken($token);
	...
	public function getOrganicGroupName($token)
	{
		$folder = $this->getFolderFromToken($token);
		if (! $this->isInstalled($folder)) {
			return '';
	...
	public function getOrganicGroupLeaderToken($token)
	{
		$folder = $this->getFolderFromToken($token);
		if (! $this->isInstalled($folder)) {
			return '';
	...
	public function getOrganicGroupPendingToken($token)
	{
		$folder = $this->getFolderFromToken($token);
		if (! $this->isInstalled($folder)) {
			return '';
	...
	public function getOrganicGroupBaseToken($token)
	{
		$folder = $this->getFolderFromToken($token);
		if (! $this->isInstalled($folder)) {
			return '';
	...
	public function getOrganicGroupLeaders($token)
	{
	...
	public function getOrganicGroupCatsForUser($usr)
	{
	...
	public function organicGroupIsPrivate($token)
	{
	...
	public function getOrganicGroupInfoForItem($type, $id)
	{
		$pubcats = [];
		$pricats = [];
		foreach (self::$public_catroots as $folder => $ref) {
	...
	public function getGroupHomePage($token)
	{
		$folder = $this->getFolderFromToken($token);
		if (! $this->isInstalled($folder)) {
			return '';
	...
	public function getGroupManagementPage($token)
	{
		$folder = $this->getFolderFromToken($token);
		if (! $this->isInstalled($folder)) {
			return '';
