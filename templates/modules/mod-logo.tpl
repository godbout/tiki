...
{strip}
	{tikimodule error=$module_params.error title=$tpl_module_title name="logo" flip=$module_params.flip decorations=$module_params.decorations nobox=$module_params.nobox notitle=$module_params.notitle}
		{if $module_params.src}
			<div {if $module_params.bgcolor ne ''} style="background-color: {$module_params.bgcolor};"{/if} class="position-relative pull-left {$module_params.class_image|escape}">
				...
				{if $tiki_p_admin eq "y"}<a class="btn btn-primary btn-sm mb-2 mr-1 position-absolute opacity50" href="tiki-admin.php?page=look&cookietab=2&highlight=sitelogo_src#feature_sitelogo_childcontainer" style="bottom: 0; right: 0" title="{tr}Edit{/tr}"><span class="fa fa-edit"></span>&nbsp;{tr}Change the logo{/tr}</a>{/if}
			</div>
		{/if}
