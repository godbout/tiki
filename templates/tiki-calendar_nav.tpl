...
{if empty($module_params.viewnavbar) || $module_params.viewnavbar eq 'y'}
	<div class="clearfix tabrow" {if $module eq 'y'}style="padding: 0"{/if}>
		<div class="viewmode clearfix">
			...
			{if !isset($calendar_type) or $calendar_type neq "tiki_actions"}
				{if $module neq 'y'}
					{button _auto_args="viewmode,focus" _text="{tr}Today{/tr}" _class="calbuttonoff mb-3" _type="xs btn-primary" viewmode='day' focus=$now todate=$now}
					<br>
				{else}
					{if empty($module_params.viewmode)}
						{button _auto_args="viewmode,focus" _keepall=y _text="{tr}Today{/tr}" _class="calbuttonoff mb-3" _type="xs btn-primary" viewmode='day' focus=$now todate=$now}
						<br>
					{else}
						{button _auto_args="focus" _keepall=y _text="{tr}Today{/tr}" _class="calbuttonoff mb-3" _type="xs btn-primary" focus=$now todate=$now}
						<br>
					{/if}
				{/if}
			{/if}
	...
	{*viewmodes*}<div class="btn-toolbar" style="display:inline-block;" role="toolbar" aria-label="...">
				{if !isset($calendar_type) or $calendar_type neq "tiki_actions"}
					{if $module neq 'y'}
						{button _ajax=$ajax href="?viewmode=day" _title=":{tr}Day{/tr}" _text="{tr}Day{/tr}" _selected_class="buttonon tips" _type="xs btn-primary tips" _selected="{if $viewmode == 'day'}y{else}n{/if}"}
					{elseif empty($module_params.viewmode)}
						{button _ajax=$ajax viewmode='day' _auto_args="viewmode" _keepall='y' _title=":{tr}Day{/tr}" _text="{tr}D{/tr}" _selected_class="buttonon" _type="xs btn-primary tips" _selected="{if $viewmode == 'day'}y{else}n{/if}"}
					{/if}
				{/if}
				{if $module neq 'y'}
