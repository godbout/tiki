...
{if !isset($versioned) or not $versioned}
	{capture assign=more_section}
		...
		{if $prefs.feature_wiki_undo eq 'y' and $canundo eq 'y'}
			<li class="dropdown-item">{button _keepall='y' href="tiki-index.php" page=$page undo="1" _type="link" _text="{tr}Undo{/tr}"}</li>
		{/if}
		...
		and $editable) and $structure eq 'n' and count($showstructs) eq 0}
			<li class="dropdown-item">{button _keepall='y' href="tiki-index.php" page=$page convertstructure="1" _type="link" _text="{tr}Make Structure{/tr}"}</li>
		{/if}

		{if $prefs.feature_slideshow eq 'y' && $prefs.wiki_uses_slides eq 'y'}
			{if $show_slideshow eq 'y'}
				<li class="dropdown-item">{button _keepall='y' href="./tiki-slideshow.php" page=$page _type="link" _text="{tr}Slideshow{/tr}"}</li>
			{elseif $structure eq 'y'}
				<li class="dropdown-item">{button _keepall='y' href="tiki-slideshow2.php" page_ref_id=$page_info.page_ref_id _type="link" _text="{tr}Slideshow{/tr}"}</li>
			{/if}
		{/if}

		{if $prefs.feature_wiki_export eq 'y' and ( $tiki_p_admin_wiki eq 'y' or $tiki_p_export_wiki eq 'y' )}
			<li class="dropdown-item">{button _keepall='y' href="tiki-export_wiki_pages.php" page=$page _type="link" _text="{tr}Export{/tr}"}</li>
		{/if}
		...
		{if $prefs.feature_wiki_discuss eq 'y' && $show_page eq 'y' && $tiki_p_forum_post eq 'y' && ( empty($prefs.wiki_discuss_visibility) || $prefs.wiki_discuss_visibility eq 'button')}
			...
			{/capture}
			<li class="dropdown-item">{button _keepall='y' href="tiki-view_forum.php" forumId=$prefs.wiki_forum_id comments_postComment="post" comments_title=$page comments_data=$wiki_discussion_string comment_topictype="n" _type="link" _text="{tr}Discuss{/tr}"}</li>
		{/if}
		{if $prefs.feature_multilingual eq 'y' and ($tiki_p_edit eq 'y'
		or (!$user and $prefs.wiki_encourage_contribution eq 'y')) and !$lock}
			<li class="dropdown-item">{button _keepall='y' href="tiki-edit_translation.php" page=$page _type="link" _text="{tr}Translate{/tr}"}</li>
		{/if}

		{if $tiki_p_admin_wiki eq 'y' && $prefs.wiki_keywords eq 'y'}
			<li class="dropdown-item">{button _keepall='y' href="tiki-admin_keywords.php" page=$page _type="link" _text="{tr}Keywords{/tr}"}</li>
		{/if}
		{if $user and (isset($tiki_p_create_bookmarks) and $tiki_p_create_bookmarks eq 'y') and $prefs.feature_user_bookmarks eq 'y'}
			<li class="dropdown-item">{assign var=urlurl value="{$page|sefurl}{$smarty.server.REQUEST_URI|regex_replace:'/^[^\?\&]*/':''|regex_replace:'/(\?page=[^\&]+)/':''}"}{button _script="tiki-user_bookmarks.php" urlname=$page urlurl=$urlurl addurl="Add" _type="link" _text="{tr}Bookmark{/tr}" _auto_args="urlname,urlurl,addurl"}</li>
		{/if}
	...
	{if $page_bar|trim neq ''}
		<div class="form-group row mx-0" id="page-bar">
			<div class="btn-bar">
				{$page_bar}
			</div>
		</div>
